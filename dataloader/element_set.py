...
import sys
sys.path.append('..')
from misc.utils import (save_model, load_model, my_logger, load_embedding,
                   load_raw_data, Results, Metrics, read_synset, write_synset, get_max_len_size, save_evaluation_result)
import os 
random.seed(5417)
np.random.seed(5417)
class ElementSet(object):
    ...
    def __init__(self, name, data_format, options, raw_data_strings=None):
        ...
        self.vocab = []  # this vocab will contain only instances that appear in the above positive_set at least once，，语料中出现的词 index表示
        ...
        self.sip_triplets = []  # [set,inst, is_pair]
        self.pos_sip_cnt = -1
        self.neg_sip_cnt = -1

        # used to generate a collection of <set, instance> pair for evaluation in advance.
        # for each positive (set, instance) pair, generate at most 10 negative pairs
        self.NEG_SAMPLE_RATIO = 10
        # for each full set, generate at most 500 positive (set, instance) pairs
        self.MAX_POS_SUB_SET_CNT = 500
    ...
    def __repr__(self):
        return "<ElementSet {} (data_format = {}, vocab_size = {}, number of sets = {}, " \
               ...
               "positive pairs = {}, negative pairs = {})>".format(self.name, self.data_format, len(self.vocab),
                                                                   len(
                                                                       self.positive_sets), self.max_set_size, self.min_set_size, self.avg_set_size,
    ...
    def _initialize_set_format(self, raw_set_strings):
        ...
        for line in raw_set_strings:
            ...
            # sorting for reproducibility {'同义词1',‘同义词2’}
            cls = sorted(list(eval(cls)))
            # print(cls)
            cls = [c.split('(')[0].replace(' ', '_')
                   for c in cls]  # 取()前的str与将空格替换为_
            self.max_set_size = max(self.max_set_size, len(cls))
            self.min_set_size = min(self.min_set_size, len(cls))
            set_size_sum += len(cls)

            # sorting for reproducibility
            self.positive_sets.append(
                sorted([self.word2index[ele] for ele in cls]))
    ...
    def _initialize_sip_format(self, raw_set_instance_strings):
        ...
        for line in raw_set_instance_strings:
            ...
            subset = sorted(list([self.word2index[ele] for ele in eval(
                " ".join(segs[:-2]))]))  # sorting for reproducibility
    ...
    def get_train_batch(self, max_set_size=100, pos_sample_method="vary_size_enumerate_with_full_set", neg_sample_size=1,
        ...
        if self.data_format == "set":
            ...
            for raw_set in self.positive_sets:
                ...
                if len(raw_sets) % batch_size == 0:  # 满一个batch_size才会执行
                    result = self._convert_set_format_to_sip_format(raw_sets=raw_sets,
                                                                                pos_strategy=pos_sample_method,
                                                                                neg_strategy=neg_sample_method,
                                                                                neg_sample_size=neg_sample_size,
                                                                                max_set_size=max_set_size)
                    if isinstance(result,tuple):
                        sip_triplets,_,_ = result
                    else:
                        sip_triplets = result
                    # sip_triplets :[([1,2,3],2,1]),...,]
                    batch_set = []  # [set1,set2,...]
                    batch_inst = []  # [i1,i2,..]
                    labels = []  # [l1,l2,...]
                    ...
                    batch = self._convert_sip_format_to_tensor(
                        max_set_size, batch_set, batch_inst, labels)
                    yield batch
                    raw_sets = []

            # yield the last batch
            if len(raw_sets) != 0:
                result= self._convert_set_format_to_sip_format(raw_sets=raw_sets, pos_strategy=pos_sample_method,
                                                                            neg_strategy=neg_sample_method,
                                                                            neg_sample_size=neg_sample_size,
                                                                            max_set_size=max_set_size)
                if isinstance(result,tuple):
    ...
    def get_test_batch(self, max_set_size=5, batch_size=32):
        ...
        for idx, batch in enumerate(self.sip_triplets):
            ...
            if (idx + 1) % batch_size == 0:
                res = self._convert_sip_format_to_tensor(
                    max_set_size, batch_set, batch_inst, labels)
                yield res
                batch_set = []
                batch_inst = []
    ...
    def _convert_set_format_to_sip_format(self, raw_sets, pos_strategy, neg_strategy, neg_sample_size=10,
        ...
        :type raw_sets: list
        :param pos_strategy: name of positive sampling method(positive起主要作用)
        :type pos_strategy: str
        :param neg_strategy: name of negative sampling method(negative 没有起什么作用)
        ...
        if pos_strategy == "sample_size_repeat_set":
            ...
            batch_neg = self._generate_negative_samples_within_pool(
                batch_fullset, neg_sample_size, remove_pos=True)
        ...
        elif pos_strategy == "sample_size_random_set":  # use
            batch_neg = self._generate_negative_samples_within_pool(
                raw_sets, neg_sample_size, remove_pos=True)
            # print("neg_sample_size: {}".format(neg_sample_size))
            # print("batch_neg:", batch_neg)
            batch_set = []
            batch_pos = []
            for raw_set in raw_sets:  # raw_set 同义词集合 [[11,12,13],...]
                if len(raw_set) == 1:  # [1]
                    batch_set.append(
                        [raw_set for _ in range(neg_sample_size + 1)])
                    batch_pos.append(raw_set[0])
                    continue
                ...
                # 目的是得到长度不一样的k_set,但为什么一个len(k_set)=1+neg_sample_size,为了下面负例的生成
                while len(k_set) != (1 + neg_sample_size):
                    # consume current pass, resample subset size
                    if start_idx + subset_size > len(raw_set_new):
                        random.shuffle(raw_set_new)
                        subset_size = random.randint(1, subset_size_range - 1)
                        start_idx = 0
                    k_set.append(
                        raw_set_new[start_idx: start_idx + subset_size])
            ...
            for idx, subset_list in enumerate(batch_set):
                for idy, subset in enumerate(subset_list):
                    if idy == 0:
                        ...
                        sip_triplets.append(
                            (subset, batch_neg[idx][idy - 1], 0))

            return sip_triplets  # [([1,2,3],2,1]),...,]
            ...
            batch_fullset = []  # used to generate negative samples
            for raw_set in raw_sets:
                # if we cannot sample a sip in which the "set" is of size subset_size
                if len(raw_set) < subset_size + 1:
                    if len(raw_set) == 1:
                        ...
                        raw_set_new = raw_set.copy()
                        random.shuffle(raw_set_new)
                        # select maximum size of subset_size
                        batch_set.append(raw_set[1:])
                        batch_pos.append(raw_set[0])
                        batch_fullset.append(raw_set)
                    continue
        ...
        elif pos_strategy == "vary_size_enumerate_with_full_set":  # use
            # print('use vary')
            sip_triplets = []
            pos_sip_cnt_sum = 0
            neg_sip_cnt_sum = 0
            ...
            for subset_size in range(1, max_set_size + 1):
                for raw_set in raw_sets:  # 每一个都是同义词集
                    ...
                    pos_sip_cnt = int(len(batch_set) / (neg_sample_size + 1))#算每个pos对应多少set，趋向于1
                    pos_sip_cnt_sum += pos_sip_cnt 
                    neg_sip_cnt = int(pos_sip_cnt * neg_sample_size) # 每个pos对应neg个neg
                    neg_sip_cnt_sum += neg_sip_cnt
                    ...
                    sample_times = int(neg_sip_cnt / sample_size) # 一般情况还是1
            ...
            for r in range(1, max_set_size + 1):
                for positive_full_set in raw_sets:
                    if len(positive_full_set) < r + 1:  # unable to sample a set of size r
                        continue
                    negative_pool = [
                        ele for ele in self.vocab if ele not in positive_full_set]
                    ...
                    neg_insts = []
                    for _ in range(sample_times):
                        neg_insts.extend(random.sample(
                            negative_pool, sample_size))
    ...
    def _convert_sip_format_to_tensor(self, max_set_size, batch_set, batch_inst, labels):
        ...
        batch_set_tensor = torch.from_numpy(
            batch_set_tensor)  # (batch_size, max_set_size)
    ...
    def _generate_negative_samples_within_pool(self, positive_sets, neg_sample_size, remove_pos=True):
        ...
        for positive_set in positive_sets:
            if remove_pos:
                sample_pool = [
                    ele for ele in self.vocab if ele not in positive_set]
            else:
                sample_pool = self.vocab

            if neg_sample_size <= len(sample_pool):
                # 如果sample_pool的容量过过大，通过抽样的方式抽一些
                neg = random.sample(sample_pool, neg_sample_size)
            else:
                repeat_time = int(neg_sample_size / len(sample_pool))
                neg = sample_pool.copy() * repeat_time  # list * int  如果不够 直接重复repeat_time
                remaining_num = neg_sample_size - \
                    len(sample_pool) * repeat_time
...
if __name__ == '__main__':
    ...
    options['neg_sample_size']=20
    max_set_size=options['max_set_size']=25

    word_embed_dict = {
            'word': 'word_vector.txt.vec',
            'fast': 'fast_vector.txt.vec',
            'visual': 'visual.txt.vec',
            'fast_visual': 'fast_visual.txt.vec',
            'word_visual': 'word_visual.txt.vec',
            'visual100':'visual100.txt.vec',
            'visual5000':'visual5000.txt.vec'

        }

        # Load embedding files and word <-> index map
    options['feat_type']='word'

    fi = os.path.join("/raid_sdb/home/chenguang/nice_tag_data/nice_synonym_data_v2/",
                          word_embed_dict[options['feat_type']])
    ...
    options['synset_path'] ='split_0'

    synset_path = os.path.join(
                "/raid_sdb/home/chenguang/nice_tag_data/nice_synonym_data_v2/", options['synset_path'])
    train_synset = read_synset(
        os.path.join(synset_path, 'train_label.txt'))
    test_synset = read_synset(os.path.join(synset_path, 'test_label.txt'))
    train_raw_data_string = write_synset(train_synset)
    test_raw_data_string = write_synset(test_synset)
    

    random.shuffle(train_raw_data_string)
    train_set= ElementSet(
        "train_set_full", options["data_format"], options, train_raw_data_string)
    print(train_set)

    # Load test_set, always in set format

    random.shuffle(test_raw_data_string)
    test_set = ElementSet(
        "test_set", "set", options, test_raw_data_string)
    print(test_set)

    x=0
    
    for i in train_set.get_train_batch(max_set_size=25,pos_sample_method="vary_size_enumerate_with_full_set",neg_sample_size=20):
        
        sets = i['set']
        print(sets.shape)
        
    
    for i in test_set.get_test_batch(max_set_size=20,batch_size=1000):
        sets = i['set']
        print(i['inst'].shape)
        print(sets.shape)
